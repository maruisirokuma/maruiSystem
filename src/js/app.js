...
/* ====================================================
   初期商品データ投入（ユーザー指定データ）
==================================================== */
const INITIAL_PRODUCT_DATA_VERSION = 3;
...
async function seedProducts() {
  ...
  const expectedNames = new Set(initialProducts.map(p => p.name));
  ...
  if (hasChanges || currentVersion < INITIAL_PRODUCT_DATA_VERSION) {
    localStorage.setItem(INITIAL_PRODUCT_DATA_KEY, String(INITIAL_PRODUCT_DATA_VERSION));
    console.log('商品マスタを指定データに同期しました');
  }
}
